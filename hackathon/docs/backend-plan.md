...
### CP4 / Phase 4：Agent B/C 与 DecisionEngine
...
> **目标**：将 Sentinel 的执行层从自研 SmartAccount.sol 切换到 Cobo Agentic Wallet (CAW)，
> 使项目完全符合 Cobo 赛道要求，同时保留 Sentinel 风控管线作为 CAW 之上的 AI 安全层。
> 方向：**04｜Autonomous Trading** — 带风控的自主交易 Agent。

### 为什么换

Cobo 赛道硬性要求：Agent 资金操作必须通过 CAW 完成。SmartAccount.sol 是自研合约，
不符合赛道规则。但反过来，CAW 解决"Agent 怎么持有和执行资金"，Sentinel 解决
"Agent 该不该执行这笔交易"，两者组合 = 完整的可信自主交易 Agent。

### CAW 核心概念（开发前必读）

- **Pact（权限契约）**：Agent 提交 pact 描述意图（如"24h 内最多 swap 0.5 ETH"），
  owner 在 Cobo app 审批后，Agent 在 pact 范围内自主执行。
- **MPC 签名**：Agent 不持有私钥，CAW 用 MPC 代替 EOA 签名。
- **Server-side policy**：CAW 服务端执行策略，超出 pact 范围的操作直接拒绝并返回
  结构化错误（跟 Sentinel 的硬规则 reject 类似）。
- **审计日志**：CAW 自带 `get_audit_logs`，可补充 Sentinel 本地审计。

### 架构变化

```
改前: 用户意图 → Agent A → RiskPipeline → Agent B/C → DecisionEngine → SmartAccount.execute()
改后: 用户意图 → Agent A → RiskPipeline → Agent B/C → DecisionEngine → CAW contract_call()
                                                                              ↑
                                                          CAW Pact 策略层（二次验证）
```

Sentinel 风控在 CAW 之前（AI 层判断），CAW Pact 在执行时（服务端策略兜底），
形成 **双层防护**：AI 风控 + 链上策略。
...
| # | 任务 | 改动范围 | 预估 |
|---|---|---|---|
| C1 | 安装 CAW Python SDK，跑通 hello-world | 新依赖 + 配置 | 1-2h |
| C2 | 创建 CAW 钱包，pairing owner | CLI 操作 + 文档记录 | 0.5h |
| C3 | executor.py 改用 CAW `contract_call()` / `transfer_tokens()` | 核心改造 | 2-3h |
| C4 | Pact 提交流程：DecisionEngine approve → 自动提交 pact → 等 owner 审批 | 新增流程 | 1-2h |
| C5 | 前端展示 CAW 钱包地址 + 交易 hash 替代 SmartAccount 地址 | 前端小改 | 1h |

#### 可选（时间允许）

| # | 任务 | 说明 |
|---|---|---|
| C6 | SmartAccount.sol 保留为"链上额外防线" | 双层合约验证，demo 加分 |
| C7 | CAW 审计日志 + Sentinel 本地日志合并展示 | 前端 audit 页更丰富 |
| C8 | 多 Agent 钱包（A2A 方向） | 每个 Agent 独立 CAW 钱包，方向 05 |

### 关键代码改动预览

```python
# executor.py — 改前
tx_hash = smart_account.execute(to, value, data)

# executor.py — 改后
from cobo_wasm import CoboClient  # CAW Python SDK

client = CoboClient(api_key=..., env="sandbox")
# 先检查 pact 是否覆盖此操作
result = client.contract_call(
    wallet_id=wallet_id,
    chain_id="sepolia",
    to=to_address,
    value=value,
...
class CoboPactResult:
    pact_id: str
    status: Literal["submitted", "approved", "rejected", "expired"]
    owner_action: Optional[str] = None  # owner 审批/拒绝的备注

# AuditRecord 新增字段
class AuditRecord:
    # ... 现有字段 ...
    cobo_pact: Optional[CoboPactResult] = None  # CAW pact 状态
    cobo_tx_hash: Optional[str] = None           # CAW 返回的交易哈希
```
...
回答：
> "CAW 的 Pact 是**静态规则**（24h 最多 0.5 ETH），Sentinel 的 AI 风控是**动态判断**
> （这个地址昨天被骗过、这个 slippage 在当前市场条件下不合理、这个合约有已知漏洞）。
> 两者互补：Sentinel 做智能判断，CAW 做最终执行保障。"

### 验收标准

- CAW sandbox 钱包成功创建并 pairing。
- `contract_call()` 能在 Sepolia 上执行 swap（或 mock）。
- DecisionEngine approve 后自动提交 pact，owner 审批后执行。
- Sentinel 风控 reject 时**不提交 pact**（CAW 层完全不触达）。
- Demo 路径：成功 swap、风控拒绝、pact 超限被 CAW 拒绝（双层拒绝展示）。
