...
- 已新增 `agent/test_intent.py`，覆盖 6 个单元测试：
  ...
  - 旧格式 transfer
  - 空 dict
  - mock `parse_intent()` 后验证 `parse_tx_proposal()`
- 验证命令：
...
- Phase 1：数据模型第一版已完成；前端 API response 模型后续在 FastAPI 阶段补。
- Phase 2：`LLM dict -> TxProposal` 的 parser/validator 子任务已完成；system prompt 更新和主流程接入后续继续。
- 下一步建议进入 Phase 3 前置切片：实现 RiskPipeline 骨架和第一条 `AmountRule`。
