...
- `Suggestion` 至少包含 `field`、`suggested_value`、`reason`。
- `AgenticLoop(max_retries=2)` 在 `reject + suggestions` 时允许 Agent A 重生成 `TxProposal`。
...
- `ENABLE_REAL_TX=false` 时不发真实 CAW transaction，只返回 dry-run / mocked result。
...
### CP4 / Phase 4：DecisionEngine + Agent B/C mock + suggestions
...
### CP5 / Phase 5：FastAPI 服务与审计日志 + attempts[]
...
- CAW transaction/request id 或 testnet tx hash 可展示。
...
| C4 | 先接 `transfer_tokens`，再接 `contract_call` | CAW 执行路径 | 2-3h |
...
- safe transfer via CAW 可稳定演示。
- Sentinel 风控 reject 时不触发 CAW。
- CAW policy denied 时可以展示 policy reason。
- Demo 中有 CAW wallet address、request id / transaction record，时间允许则补 testnet tx hash。
