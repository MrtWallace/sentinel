...
| DeepSeek Agent B/C 后移 | 先保证 CAW 真实执行，降低 LLM 不稳定性 | 0.25h |
| 失败预案 | demo 当天可控 | 0.5h |
...
- CP4 先实现 `MockReproposalAgent`，按 `rejection_code` 选择策略；后续真实版用 DeepSeek/Agent A 重新推理。
...
- DeepSeek Agent B/C 后移到 CAW 真实执行之后，MVP 可继续使用 mock reviewer。
